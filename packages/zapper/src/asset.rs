...
use cosmwasm_std::{
    to_json_binary, Addr, Api, BankMsg, Binary, Coin, CosmosMsg, DepsMut, Env, MessageInfo,
    QuerierWrapper, Uint128, WasmMsg,
};
...
impl Asset {
    ...
    pub fn add(&mut self, amount: Uint128) -> Result<Uint128, ZapperError> {
        match self {
            Asset::Native(coin) => {
    ...
    pub fn sub(&mut self, amount: Uint128) -> Result<Uint128, ZapperError> {
        match self {
            Asset::Native(coin) => {
    ...
    pub fn transfer_amount(&self, amount: Uint128, to_address: &str) -> CosmosMsg {
        match self {
            Asset::Native(coin) => CosmosMsg::Bank(BankMsg::Send {
                to_address: to_address.to_string(),
                amount: vec![Coin {
    ...
    pub fn into_wasm_msg(self, contract_addr: String, msg: Binary) -> Result<WasmMsg, ZapperError> {
        match self {
        ...
        match self {
            Asset::Native(coin) => {
                ...
                if compare_coin.eq(coin) {
                    ...
                    Err(ZapperError::InvalidNativeCoin)
                }
            }
            Asset::Cw20(coin) => {
                ...
                if coin.amount <= balance {
                    ...
                    Err(ZapperError::InvalidCw20Coin)
                }
            }
        }
    }
...
pub fn get_current_asset_available(
    ...
    address: &Addr,
    denom: &str,
) -> Result<Asset, ZapperError> {
    match api.addr_validate(denom) {
        Ok(addr) => {
            let cw20_contract = Cw20Contract(addr.clone());

            let amount = cw20_contract.balance(querier, address)?;
...
mod tests {
    ...
    fn test_validate_native() {
        ...
        assert_eq!(res, Err(ZapperError::InvalidNativeCoin));
        ...
        assert_eq!(
            res,
            Err(ZapperError::Payment(PaymentError::MultipleDenoms {}))
        );
    }
    ...
    fn test_validate_cw20() {
        ...
        assert_eq!(res, Err(ZapperError::Payment(PaymentError::NonPayable {})));
        ...
        assert_eq!(res, Err(ZapperError::InvalidCw20Coin));
    }

    #[test]
    fn test_get_current_asset_available() {
        // TEST 1: Native asset
        let deps = mock_dependencies_with_balances(&[("entry_point", &[Coin::new(100, "ua")])]);

        let mut env = mock_env();
        env.contract.address = Addr::unchecked("entry_point");

        let asset = get_current_asset_available(
            deps.as_ref().api,
            &deps.as_ref().querier,
